...
适用于 R / Python 流行病学与生物统计工作的跨任务规则。这里只保留每次会话都需要的任务分流、硬红线、唯一来源指针、优先级和完成条件；具体流程、模板、参数与示例由对应 skill 及其 references 按需加载。

## 1. 任务模式与最短分流

- 开工先读适用规则；标准项目再读 `BACKLOG.md`。先判工作区，再判本轮范围：问答只回复且不创建文件；局部产物只改一个已锁定文件、部件或格式项；项目工作只同步实际受影响的数据链；明确投稿、外发或归档才进入正式发布。保留来源不明的既有改动，多个合理当前版无法判定时先问用户。
- 非标准项目的简单作业、单次处理或快速核验走轻量通道，不自动初始化目录、registry、`results.yaml` 或项目账本。标准项目内的局部产物也不因此触发整项目清理、日志补写或六层审查；领域 skill 的触发不自动扩大范围。
...
- 正式项目使用相对路径；活动目录和正式产物在创建前登记到 `.epiagentkit-layout.json`。`09_backup/workbench/` 只登记基础目录，批次内临时脚本、诊断、测试和探索内容由 `PLAN.md`、`FINDINGS.md` 管理，不逐文件登记，也不写系统临时目录或项目根。轻量任务保持输入只读、输出集中、命名清楚，不迁移用户既有目录。
...
问答与局部产物以用户锁定的输入、范围和输出为准，不补建 `results.yaml`、修订状态卡或项目账本；只有本轮确实改变结果、方法、正式当前稿或待办时才同步对应唯一来源。
...
- 问答完成时只确认问题已直接回答；局部产物完成时确认唯一输入和授权范围已锁定、原文件可恢复、输出可打开、范围外差异为零，不处理无关既有问题或补建记录文件。
- 项目工作完成时确认原始数据未改，并只同步实际受影响的结果、方法、状态与 registry；正式发布再确认当前交付物使用稳定语义名且只留一组，旧版可恢复归档。
- 仅在正式项目审查或正式发布前运行 `python <epi-project-audit技能目录>/scripts/run_check_project.py <项目根> --json`；ERROR 阻止发布，WARN 逐项解释。局部产物和日常步骤只做与本轮风险相称的验证。
