...
以原生 SVG 构建内容精确、可编辑的非统计图形。泛化的流程图、框架图、机制图、示意图和跨载体配图由 `research-visuals` 先调用 imagegen；只有用户明确要求矢量、当前会话没有可用图像生成工具，或生成图无法通过内容精度门禁时才进入本技能。默认不调用 R 绘图包生成流程框，也不把 HTML、Mermaid 或 Graphviz 默认主题截图当成品。
