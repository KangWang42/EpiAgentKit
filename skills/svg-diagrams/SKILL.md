---
name: svg-diagrams
description: 为论文、报告和 PPT 提供可编辑、内容精确的 SVG 非统计图解及 imagegen 回退路径，包括流程图、技术路线图、结构图、层级图、包含关系图、概念框架、机制示意图、时间线、矩阵和系统架构图。仅在以下场景触发：(1) 用户明确要求 SVG、矢量源或完全可编辑图形；(2) image-diagrams 已核实实际 imagegen 不可用或真实调用失败；(3) imagegen 完整成图经定向修正或整图重生成后仍无法保证文字、数字、关系和方向准确；(4) 审校或修改现有 SVG。泛化图解和 PPT 配图必须先走 image-diagrams；文字数字密集或当前技能清单未列出 image-diagrams 均不是 SVG 触发条件；统计数据图走 publication-figures。
---

# SVG 图解回退

以原生 SVG 构建内容精确、可编辑的非统计图形。泛化的流程图、框架图、机制图、示意图和 PPT 配图由 `image-diagrams` 先调用 imagegen；只有用户明确要求矢量、当前会话没有可用图像生成工具，或生成图无法通过内容精度门禁时才进入本技能。默认不调用 R 绘图包生成流程框，也不把 HTML、Mermaid 或 Graphviz 默认主题截图当成品。

进入本技能后先记录回退原因，并沿用 `image-diagrams` 已锁定的节点、标签和关系契约；不得在工具切换时改动信息结构。用户显式调用本技能或直接要求 SVG 时无需先生成位图。
...
“文字或数字多”“预计 imagegen 可能写错”“SVG 更容易做”“当前可见 skills 清单没有 `image-diagrams`”均为无效凭证，不得据此提前进入本技能。
...
2. 选择视觉配置：队列筛选、样本纳排、CONSORT 或病例流转使用 `journal-flow`；概念机制、技术路线、层级结构、包含关系、时间轴、矩阵和系统架构使用 `editorial`。不得把多色概念卡片用于研究对象筛选流程，也不得把队列蓝灰框机械套到其它图类。
3. 选择拓扑：优先从 A 层级汇聚、B 谱系时间轴、C 嵌套包含中选择；语义不匹配时再用线性流程、矩阵、机制或架构图。不得先选好看的模板再硬塞内容。
4. 分配语义样式：所有图先采用期刊队列图的共同语法，即白底、细边框、低饱和浅填充、常规正文和几何优先。`journal-flow` 固定使用浅蓝主路径与灰色排除框。论文 `editorial` 强制使用白/灰加单一蓝色强调，不按 biology、exposure、risk、outcome 等类别自动换色；内部 PPT 只有在颜色确实承担不可替代的语义时才增加类别色。
...
| 图类 | 默认结构 | 主要视觉线索 |
| --- | --- | --- |
| 队列筛选、CONSORT | `journal-flow` 纵向主线或末端分支 | 蓝色主框、灰色排除框、近直角 |
...
- 论文 `editorial` 只允许一个蓝色相加白/灰中性色；即使存在多个语义类别，也用标题、位置、边界和连线区分，禁止黄、红、绿、橙分别映射变量类别。报告和内部 PPT 默认同样执行，用户明确要求多类别色且颜色确有解释价值时才放宽。
...
- 画布：`data-role="canvas"`
- 卡片或嵌套层：`data-role="card|nested-layer" data-category="..." data-layer="..."`
- 期刊研究流程：`data-role="flow-main|flow-exclusion|flow-terminal" data-layer="..."`
- 标题与副标题：`data-role="node-title|node-subtitle" data-category="..."`
- 连线、主轴与刻度：`data-role="connector|axis|axis-tick"`
- 可选 PPT 装饰：`data-role="decoration"`；论文图禁止该角色。

语义类别固定为 `biology`、`exposure`、`covariate`、`risk`、`outcome`、`nonlinear`，具体颜色见设计系统。

## 与论文、报告和 PPT 的衔接

- 统计图、森林图、生存曲线、热图等仍由 `publication-figures` 生成；本技能不处理数据映射图。
- 流程、结构、机制、路线、包含关系和 PPT 非统计配图默认由 `image-diagrams` 调用 imagegen；本技能承担显式矢量需求和失败回退，不再作为泛化图解的默认入口。
...
- [ ] 论文图只有一个蓝色相加白/灰中性色，无按变量类别铺开的五色卡片
