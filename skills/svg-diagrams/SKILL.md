...
先按全局 `CLAUDE.md` 判定范围：Q 只解释或只读核对指定内容；L 只修改一个已确认的 SVG、节点、标签、连线或显示项；P 新建图件或重大重绘；R 再按论文、报告、PPT、网页或归档要求检查正式当前版。L 不重新规划整张图，不为一个局部修改执行完整图类清单或重查无关节点。
...
8. 按范围验证：P/R 按视觉配置运行 `scripts/validate_svg.py --profile journal-flow|editorial`。内容精确图同时传入全部必需节点、必需边和 `--require-semantic-graph`；独立单标题图加入 `--single-title`；论文或报告 PNG 预览加入 `--preview-png`、`--target-width-mm` 与 `--target-ppi`。L 只使用能够发现本次修改错误的参数，并核对目标变化、未授权文字和关系未变、实际使用该图的论文页、报告、PPT 或网页已更新；不重复与本次修改无关的完整语义图、分辨率或全图样式检查。随后在最终嵌入尺寸检查受影响位置，修复后只重新验证该问题和实际受其影响的成品。
...
- 报告图：按 Word 版心宽度设计，通常 140–165 mm；保留同名 PNG，供 python-docx 无法直接使用 SVG 时嵌入。
- README 与技术文档：按实际内容列宽设计；正文内容图直接包含必要标签，并在桌面与窄屏渲染下检查字号、箭头、替代文本和文件体积。
- 所有使用位置：内容决定高度，避免为了填满固定比例制造大面积空白。
...
- `editorial` 默认使用两类低饱和语义主色，必要时增加一类警示或关键状态色；主色相总数不超过三种。即使存在多个语义类别，也先用标题、位置、边界和连线区分，不按变量类别逐框随机换色。目标文档或网页有既有视觉系统时，在保持语义角色不变的前提下适配其颜色。
- 同层节点等宽等高；同一行列使用共同的对齐基准；相邻间距一致；分支使用正交或短曲线连接，避免长斜虚线跨越大片空白。
...
- 中文字体统一使用思源黑体、苹方或微软雅黑之一；英文使用 Arial/Helvetica。SVG 中按顺序列出可替代字体，不依赖某一台电脑上的单一字体。
...
以下项目只选择与图类、使用位置和本次变化实际有关的内容；它们适用于 P 新图、重大重绘和 R 正式发布，不是 L 局部修改的固定清单。每项检查必须能指出可能发现的具体内容、关系、显示或文件错误，否则不执行。
...
- [ ] 正式项目当前版使用稳定文件名；确有需要恢复的旧正式版已整组归档，轻量任务不建立项目归档

L 完成时只确认：指定节点、标签、连线或显示项已经按要求修改；其它文字、数字、关系和样式未被误改；SVG 可重新打开；实际使用该图的论文、报告、PPT 或网页已更新。只有共享样式、画布比例、语义图结构、输出尺寸或正式用途发生变化时，才扩大检查。
