---
name: svg-diagrams
description: 为论文、报告和 PPT 生成或审校克制、专业、留白充分的 SVG 原生非统计图解，包括流程图、技术路线图、结构图、层级图、包含关系图、概念框架、机制示意图、时间线、矩阵和系统架构图。触发场景：(1) 用户要求画流程、结构、关系、框架、路线或示意图；(2) 现有图存在模板化视觉痕迹、卡片墙、装饰过多、对齐、比例、留白、字体、连线或层级问题；(3) 论文、报告或 PPT 需要可编辑矢量图。统计数据图走 publication-figures；照片、插画和纹理走 imagegen。
---

# SVG 图解

以原生 SVG 构建接近期刊插图和克制产品图解的非统计图形。默认不调用 R 绘图包生成流程框，也不把 HTML、Mermaid 或 Graphviz 默认主题截图当成品。
