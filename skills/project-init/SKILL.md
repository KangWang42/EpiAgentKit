...
description: |
  ...
  触发场景：(1) 用户明确说"创建项目"、"初始化"、"新建项目"；(2) 用户明确要把空工作区建成标准研究项目；(3) 接了咨询任务并明确要求新建项目。简单作业、单次处理、快速核验、仅需少量输出或已有项目开始分析不触发本 skill。
...
【项目初始化自检】
  ...
  - Git 仓库已初始化；完整请求验证通过后按全局偏好自动 commit，用户明确要求时才 push
...
| 不在 git 仓库但用户要启 git | 执行 `git init`；完成初始化与验证后按全局偏好自动首次 commit；仅在用户当轮明确要求时 push |
