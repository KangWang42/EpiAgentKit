...
2. **隔离试验**：在 `09_backup/<YYYY-MM-DD>_<主题>/` 下新建独立实验脚本，**复用**主流程的数据集、特征构造、CV 划分、bootstrap、口径常量，**只改要试的那一个变量**（分类器 / 特征集 / CV 方案等）。Git 可用且当前目录为仓库时，多文件或跨多轮的高侵入实验可再用 `experiment/<主题>` 分支隔离；Git 不可用时只使用实验目录，不初始化仓库也不安装 Git。分支只承担隔离，不改变全局 Git 收尾策略。
