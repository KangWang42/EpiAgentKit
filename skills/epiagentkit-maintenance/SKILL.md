...
双成果的验收标准由用户原始目标和目标 skill 的正式交付约束决定，不能为了缩短工作量退化成最小 smoke test。用户提出“完整、正式、可存档、可提交、与范本同等质量”时，必须先把这些词转成可检查的内容、证据、版式、文件格式和深度要求，再按该标准制作两份成果；章节标题命中、短提纲、占位表、单页预览、验证日志或“文件能打开”都不能替代目标质量证明。
...
长论文、长报告、完整教材或其他一次难以稳定完成的产物，可以分块制作，但必须先建立逐节 todo 或内容矩阵，记录每一部分的功能、唯一输入、证据、依赖、目标格式、完成状态和验收结果。按依赖顺序逐块完成并单独检查，再装配为一个当前版，统一核对术语、数字、引文、编号、表图、交叉引用、目录和分页。分块是保证质量的执行方式，不能把局部草稿、未关闭 todo 或多份碎片文件包装成完整成果；只有所有适用部分关闭并通过整体检查，才计入双成果门槛。
