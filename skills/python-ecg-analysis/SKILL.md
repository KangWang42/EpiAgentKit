...
1. 项目 `AGENTS.md` 与 `CLAUDE.md`，确认规则唯一来源、原始数据根、锁定终点与身份键。
...
规则、README、代码或已安全核验的 `--help` 不一致时先报告，不猜命令。无法证明入口的帮助调用无副作用时，不执行它，改为从源码和项目文档核对参数。原始数据位置、输出目录、导联、采样率、轮次和特征集均以项目文件中锁定的记录为准。
...
7. 结果变化同步项目结果数字唯一来源，方法变化同步 `DECISIONS.md`，操作同步 `SESSION_LOG.md`，新缺口同步 `BACKLOG.md`。
