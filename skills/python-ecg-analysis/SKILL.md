...
description: |
  ...
  配合规则：开工前对齐 biostat-principles；实际生成统计图时使用 publication-figures。
---
...
- L 读取项目规则、目标脚本/配置和直接定义该输入、参数或输出的文档；只修目标并验证受影响记录、患者、轮次或结果，不强制读取全部 PROTOCOL、SAP、DECISIONS 和 `02_code/`。
- P/R 再按顺序完整读取：

1. 项目 `AGENTS.md` 与 `CLAUDE.md`，确认规则唯一来源、原始数据目录、已经确认的终点和记录标识字段。
2. 项目 `README.md`、`PROTOCOL.md`、`SAP.md` 与 `DECISIONS.md`，确认研究设计、轮次、时间窗和预设方法。
3. `02_code/` 的实际脚本与配置；先静态检查主程序、参数解析和 `if __name__ == "__main__"` 保护。只有确认 `--help` 会在业务逻辑前退出且不会写文件时，才执行 `python <script> --help`。
...
- `subject_id` 及其他身份标识字段全程保持字符串，保留前导零，不从行号或文件顺序推断身份。
- 在处理前验证主要标识字段的唯一性、重复记录、同一患者轮次数、采集时间顺序和一对多关系。
- 所有 `join` 明确指定连接字段，并核对连接前后的行数、未匹配记录和重复扩增；不得按行顺序拼接。
- 每个预处理产物、特征表和结局连接表记录源文件、源记录标识、参数、生成脚本与运行时间，确保可以追溯到来源和处理步骤。
...
2. 使用从源码、项目文档及已经确认不会写文件的帮助命令中核实的真实命令运行，不凭本 skill 猜测应该运行哪个脚本；首次运行前确认输出位置与原始数据只读边界。
3. 保存本轮实际运行命令的完整 stdout/stderr，并全量扫描 `error|warning|traceback|failed|nan`；不为 L 重跑无关脚本以制造全项目日志。
4. 对每项异常定位为代码、数据、阈值或库问题；修复后重跑，不能用“失败比例低”带过。
5. L 核对受影响步骤的文件数、记录数、患者数、轮次数、未匹配数，以及输出字段、类型和层级结构；P/R 再覆盖完整受影响数据链。
6. 对表图和模型结果做范围、单位、方向、泄漏与重复性检查；实际生成统计图时调用 `publication-figures`。
7. 项目执行和正式发布的关键结果由实际生成结果的脚本写入 `results/results.yaml`；方法变化同步写入 `DECISIONS.md`；总运行脚本自动保存命令、状态、日志和环境信息。只有需要后续补充材料、外部资源或由用户决定的事项才进入 `BACKLOG.md`。
