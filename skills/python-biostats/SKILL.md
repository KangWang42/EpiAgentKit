...
- 普通 Python 包缺失时按上游依赖政策在项目隔离环境补齐；失败不得静默换包、换方法或换语言。
...
方法选择或方案偏离写入 `DECISIONS.md`；总运行脚本自动记录实际命令、状态、输入输出文件哈希值和环境信息。只有需要后续补充材料、外部资源或由用户决定的事项才写入 `BACKLOG.md`。试新方法按上游 E0/E1/E2 规则隔离。
