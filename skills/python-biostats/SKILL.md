...
4. 只读检查 Python 运行时、项目依赖管理方式与所需包；复用已有兼容项目环境。普通 Python 包缺失时按 `../biostat-principles/references/runtime-dependencies.md` 优先补齐；运行时、系统依赖、非官方来源或不兼容升级先征得用户同意。
...
| 表图 | pandas/openpyxl、matplotlib/seaborn | 数字唯一来源、最终尺寸、字体与输出可打开 |

## 5. 完成条件

- 脚本在项目隔离环境中从声明输入实际运行到声明输出，新增包的名称、版本、来源与环境已记录。
- 原始输入未改，样本损失链和分析样本量已核对。
- warning、nan、收敛、模型假设和异常估计已逐项解释或修复。
- 结果、表图、解释和方法方向一致；观察性研究未使用超出证据强度的因果措辞。
- 正式项目的结果数字唯一来源、方法决策、日志和待办已同步；轻量任务报告最小复现方式。
