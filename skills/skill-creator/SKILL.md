---
name: skill-creator
description: Create or update an effective agent skill with clear routing, concise progressive disclosure, reusable resources and regression-safe validation. Use whenever a user asks to add, revise, optimize or package a skill.
...
For maximum portability (the Agent Skills open standard), `name` and `description` are the only required fields. When targeting Claude Code specifically, these optional fields are also supported and useful:

- `allowed-tools`: Restrict which tools the skill may use (e.g. `Read`, `Edit`, `Grep`) — for skills that should never run shell commands or write files.
- `disable-model-invocation: true`: Make the skill user-invocable only (`/skill-name`) — for workflows with side effects that should not auto-trigger.
- `license`, `metadata`: Informational; ignored by triggering.

Do not invent fields beyond these.
