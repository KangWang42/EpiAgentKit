...
name: report-writing
description: |
  专业中文报告内容写作技能：把分析结果、流程说明、项目进展、操作指引等转成面向读者或决策者的报告正文，并可按需生成 Markdown/Word。
  借鉴学术写作的严谨（零编造、数据有源、学术书面语、无模板化写作痕迹），但采用报告体例而非论文规范——
  结论先行、执行摘要、面向行动、结构灵活、可用要点与表格。
  触发场景：(1) 用户要"写报告/报告初稿/工作报告/分析报告/进展报告/说明文档/指南/手册/操作指引/备忘/纪要"，无论只要正文还是文件；(2) 生成非论文、非投稿的解说性、汇报性或操作性 Markdown/docx；(3) 据 0_result_summaries.md 或分析结果起草报告而非论文。
  只要正文时不调用 docx；实际创建或修改 Word 文件时再把 docx 作为文件操作 companion。
...
3. **无模板化痕迹与零装饰符号**：不堆开场闭场套话，不用表情符号或长破折号，不出现生成过程、助手口吻或未完成标记。
   中文文风统一过 `academic-humanizer` 的不可变事实清单、学术语体与论断证据审校。
4. **正文以完整段落叙述为主（强制要求）**：分析 / 结果 / 讨论 / 结论一律写成连贯的成段散文，像真实统计报告一样把数字
   织进句子里逐一解读（"S2 组调整后变化为 1.24 kg（95%CI：0.21，2.26），组内 *P* = 0.019，提示…"）。
   **NEVER** 把正文写成项目符号堆叠（一连串 `- xxx` / `• xxx` 短句 = 报告不合格）。项目符号只允许用于
...
8. **疑点先问**：读者对象 / 报告目的 / 关键口径 / 结论方向不明 → 先问用户，不擅自定调（同 CLAUDE.md §5）。
9. **局部修改也走完整标准**：哪怕只改一段，动后按 §五自检清单复扫，确保与全文术语、口径、编号一致。
10. **当前版单一、旧版整组归档**：报告当前目录只保留一组稳定同名 `.md` + `.docx`，不得生成“完善版 / 最终版 / v2 / final”等并列文件。重生成前先把被替代的 md、docx、生成脚本、相关素材与核验输出按原相对目录移入 `09_backup/YYYY-MM-DD_HHMM_<报告主题>_<阶段>/`，写该批次 `MANIFEST.md` 并登记 `09_backup/INDEX.md`；生成脚本始终输出稳定文件名。
...
- [ ] 当前目录仅一组稳定命名报告；无“完善版 / 最终版 / v2 / final”等并列旧版；归档批次的 `MANIFEST.md` 与 `09_backup/INDEX.md` 已更新
- [ ] **正文为完整段落叙述，无项目符号堆叠**（仅 SOP/纯枚举处少量使用项目符号）
...
- [ ] 中文书面语、零口语、无模板化痕迹和装饰符号；术语首现有解释；全文字体纯黑
