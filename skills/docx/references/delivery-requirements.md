...
`academic_display` 统计表应优先采用白底、无竖线的三线表结构；父级行与子级行的层级须可由缩进或明确的行分组识别。表题使用描述研究关系的科学标题，表注独立说明分析分母、事件数、效应尺度、参考组、单位和各模型实际调整变量。内部变量编码、程序状态、运行状态或版面选择说明不得写入正式 Word 表格。DOCX 只验证结构和显示，统计内容仍需由内容 skill 按稳定行键与字段 ID 逐字段对账。
