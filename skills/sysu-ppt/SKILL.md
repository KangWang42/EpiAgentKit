...
① 字体对（宋体/Times）；② 不空半屏、不溢出（表格列不被截断）；③ **图内文字够大且逐字准确**——流程图/框架图框内字、坐标轴、注释在缩略图上仍清晰可读，偏小或错误立即回到 imagegen 完整图修正或重生成；④ **图无多余空边**——图四周没有明显空白条，有则调 imagegen 构图、SVG 画布或统计图输出比例重渲染；⑤ **非统计图关系正确**——节点、箭头、方向和层级与来源逐项一致，无伪文字、重复节点或无语义装饰；走 SVG 时同行/同列/嵌套框共用锚点；⑥ **表格上下居中**——表格块在内容区垂直居中而非贴顶；⑦ 图居中不变形；⑧ 重点已加粗；⑨ 标题元素与模板设计对齐；⑩ **图内无标题、无解释性句子**（只剩坐标轴/图例/数据标签），讲道理的文字都在正文或题注；⑪ **每张图都有"图N ……"题注**，图号按出现顺序连续；⑫ **图例简短**；⑬ **学术书面**（见硬性规范 9）。
