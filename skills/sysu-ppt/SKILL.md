...
**模板适配（两类通用）**：官方模板只锁定画布、母版、校徽/水印、字体、主色、标题与页码等品牌组件，不锁定每页必须采用的内容结构。先按内容判断顺序、比较、层级、时间、证据或场景关系，再选择或组合最合适的版式，调整图文比例、区域数量、视觉权重、留白和元素尺寸。模板导致文字过小、关系被隐藏、比较失衡或内容被迫做成卡片墙时，必须重排页面或拆页，不得为“套模板”牺牲科学表达与可读性。
...
3. **内容驱动版式**：一份 PPT 混用 ≥4 种版式，但版式数量不是目标；每页按内容拓扑选择纯文字、图文、表格、公式、比较、时间轴、流程或整图。不要整份纯文字，也不要把“每页有图”当目标。图像只用于结构、机制、证据、时间或操作关系，能用表格、公式、编号短句或时间轴更清楚时就不生成插图。
