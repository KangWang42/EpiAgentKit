# ============================================================
# 开题 / 答辩 / 正式研究汇报 PPT 示例结构（区别于组会分享 deck_skeleton.R）
# 蓝本从 5 份真实开题答辩 PPT 提炼共性结构、用词与版式，按任务需要选用。
# 生成前先按 slide-design-practice.md 写逐页设计说明；本文件提供体裁结构示例，不替代逐页设计。
# 体裁特征（与组会 deck 的关键差异）：
#   1. 页数由用户要求、答辩时长、研究设计和证据量决定；方法通常是主体，但占比不固定。
#   2. 目录和章节分隔页只在用户、院系或既有模板要求，或长篇结构确有需要时使用。
#      需要时用 sysu_add_section()；组会仍禁止章节页。
#   3. 下列顺序反映常见的内容关系，不是固定逐页清单；没有实际内容的部分应删除，内容较少的相邻页应合并。
#   4. 技术路线图只在能准确统领研究内容且任务需要时保留，不为形式完整强制生成。
# ============================================================
#
# ---------- 参考模块（按用户要求与内容选用，保留必要的论证先后关系） ----------
...
#   不追求每页配图。背景证据、机制、随访/处理流程和技术路线需要图；
#   目的、定义、公式、创新、可行性和计划页优先用文字、表格、公式或时间轴。
#   科研图解先读 research-visuals/references/research-figure-patterns.md，逐页判定主要表达方式。
# ============================================================
...
# ============ 一、立题依据 ============
ppt <- sysu_add_section(ppt, "一、立题依据")     # 示例：仅在本任务需要章节分隔页时保留
