...
- 统计工作表按复核和阅读问题组织，而不是照搬程序输出。术前、术后或其它时间点可用父级行、子级行和真实缩进分组；同一变量的原始值、分组值或重复编码只在研究目的需要时同时保留。内部检查字段、变量编码、程序状态和模型运行状态不得出现在面向读者的可见工作表中。
...
- Excel 复核表可保留便于核对的缩进、分组和适度合并，但不强制模仿 Word 三线表；格式调整不得改变数据、分析分母、缺失显示或字段映射。若工作表作为正式读者表，表头、单位、参考组、`n/事件数`、效应量、95% CI 和 P 值须能独立理解，并如实保留不同指标因缺失产生的不同 `N`。
