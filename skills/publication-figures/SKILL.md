...
**范围边界**：本技能处理统计图、数据图和含坐标/尺度映射的论文结果图。流程图、技术路线、结构图、包含关系图、概念框架和机制示意等非统计图形一律调用 `svg-diagrams` 原生生成 SVG，不再用 ggplot、ggflowchart、DiagrammeR 或 R 包默认样式绘框。混合图先分别生成统计面板与 SVG 图解面板，再统一字体、配色和边距拼装。
...
- **统计图默认同时输出 PDF（矢量）+ PNG（300 DPI 位图）**，`04_figures/` 内同名不同后缀；非统计图解按 `svg-diagrams` 输出 SVG + 同名 PNG 预览。
