...
- 沿用现有包、对象命名和脚本结构；外层对象优先按研究主题、时间、分析角色或产物命名，只抽取确有重复或高风险的逻辑，不为短任务过度封装。
...
- 新增或修改的 R 代码已按 `code-style.md` 完成可读性检查；研究对象式命名、分析主线、分节、函数边界、检查位置、重复工具和运行输出均有可核对结论。仅有语法通过或全链成功不能替代这项检查。
