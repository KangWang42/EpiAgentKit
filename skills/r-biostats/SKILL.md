...
**遇到新问题**：先读 `?函数名`、warning 全文和最小复现，再检查数据与包文档；代码问题自行定位并修复。普通 R 包缺失时先按共享依赖流程安装到项目隔离环境并重跑；系统依赖、非官方来源或不兼容升级先征得用户同意，安装失败不得静默换包或换方法。只有问题涉及分组、终点、纳排、主方法口径、缺失外部输入或依赖授权边界时才问用户，不用询问代替排错。
