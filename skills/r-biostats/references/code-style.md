...
项目中可以继续使用 `fs`（`path()`、`dir_ls()`、`path_file()`）、`str_glue()`、有序 `factor()` 和 `compareGroups::descrTable()` 等已有写法。包选择仍以可复现和团队可读为先；`bruceR::set_wd()` / `import()` 等便利函数若引入过多依赖或隐藏文件位置，正式项目优先使用项目既有的相对路径方案和明确的读写函数。
...
| `descrTable() |> export2word()` | 正式项目可能已经规定 xlsx 格式、表图编号表与编号方式 | 服从项目输出要求 |
