...
先根据实际改动和直接消费者选择最小有效证据，不按“网页任务”统一运行全部检查。项目级 `CLAUDE.md`、发布流程或合规要求明确规定的命令优先，通用快速路径不能覆盖它们。
...
| P 页面或前端系统 | 新页面、整页改版、共享 token/布局/组件、新交互、路由、依赖或构建行为 | 受影响包的格式化、类型、lint、测试与必要构建；代表性页面、状态、桌面和手机视口；完整技术与视觉门禁 | 与改动无消费者关系的其它包、页面和状态 |
...
L 任务中，共享选择器或局部组件只扩大到它的直接消费者；只有修改共享 token、页面外壳、路由或组件契约时才升级到 P。用户明确接手实际页面的视觉验收时，记录 URL、状态、视口或设备以及用户确认的具体角色；该确认只替代本轮局部视觉截图，不替代语法、结构、运行时或健康检查，也不能证明其它页面和状态通过。
...
4. 第一份基线必须由责任人按组件合同确认。后续失败保留期望图、实际图和差异图；只有确认变化有意且页面消费者已检查后才更新基线，禁止在测试失败时自动接受全部新截图。
...
L 局部任务只比较受影响角色、直接消费者和不变范围，不填写整张全站视觉表。用户已在实际页面确认观感时，把其确认与自动技术检查分别记录；不再为同一局部改动默认补做多组截图，也不把用户确认外推为未查看页面、状态或视口的视觉通过。
