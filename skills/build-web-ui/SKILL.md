...
3. 用真实浏览器覆盖页面合同。至少检查约 1440 像素桌面和 CDP 仿真的 390 像素手机布局视口；新页面、整体改版或用户点名手机体验时，再检查 320 与 430 像素或页面合同中更危险的边界宽度。记录 `innerWidth`、`clientWidth`、`scrollWidth` 与 `body.scrollWidth`。不得把 `--window-size=390` 的外窗宽度或被裁切的窄截图当作 390 像素移动端证据。
4. 每类页面至少检查首屏，以及一个锚点、滚动深处或任务状态。根据项目覆盖真实长文本、筛选表单、空/错/加载状态、弹窗、后台列表和编辑页；公开页与后台页分别验收。
5. 每个浏览器场景记录 H1 数量、重复 ID、缺失 alt、破图、横向溢出、关键布局容器的左右间隙与中心偏移、核心内容越界，以及控制台异常、脚本异常、HTTP 失败和网络加载失败。截图与机器结果使用同一 URL、状态和视口。
6. 分开给出技术门禁和视觉门禁结论。技术门禁覆盖构建、DOM、资源、运行时、视口、键盘、触控和对比度；视觉门禁按同一组截图核对视觉合同、首屏重心、表面层级、色彩职责、组件重复、长内容密度和移动端重排，并对照浏览器报告中的布局几何逐层检查页面外框、章节与主要组件。正文左对齐不算偏左；未声明且无内容依据的左右安全区失衡、桌面偏移残留或多区块共同偏向一侧判为失败。有意非对称只有在视觉合同已声明、网格可追踪且手机主任务和安全区成立时通过。任一合同禁止项命中时，视觉门禁失败；机器报告零问题不得改写为视觉通过。
...
- 视觉合同说明主题来源、正向特征、反向边界、视觉重心、页面骨架、内容框与角色分工；配色按表面、文字、操作、语义和数据职责组织，关键对比度已计算，没有无职责的竞争色相。
- 组件覆盖实际需要的状态、键盘、触控、加载/失败恢复和减少动效，交互反馈流畅且不依赖单一输入方式。
- 桌面与 CDP 手机布局视口下，首屏、深层内容和关键状态可读可操作；关键容器的左右安全区、对齐线和视觉重心符合合同，无未解释的偏移、溢出、裁切、破图、控制台或网络失败。
