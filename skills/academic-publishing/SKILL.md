...
- **完整论文**：用户要求“生成论文”“写论文”“完成稿件”或同等含义时即进入本模式，不另等用户补充“完整”二字。以能够进入投稿准备为目标，先逐项列出每个结果论断、方法事实和引文的来源，再按章节功能完成内容完整的正文并做全文一致性终审。正文段落必须完成与其位置相称的科学论证，不得用一两句或几十字的短段代替方法细节、结果关系、文献比较、解释和边界。章节标题齐全、数字能够自动插入或文件已经生成，都不能单独证明论文已经完成。不得因通用流程强制拆成多轮审批，也不得擅自降级为提纲、短版骨架或自动取数模板。
