...
7. **证据约束的自然学术文风**：所有语言先过 `academic-humanizer` 的不可变事实清单与论断—证据一致性审查；中文再过
...
图须用英文标签**重出**（改 `publication-figures` 的标签映射为英文再 `Rscript 文件.R` 重跑），表头与分类标签译英。
治本做法：标签映射在 `conventions.R` 维护**中英对照**（`label_zh` / `label_en`），出图/制表按当前语言取，换语言只切语言键、不手改每张图。开工时先确认现有图表语言与目标稿语言是否一致，不一致先列出需重出的图表。
...
2. `07_paper/results.yaml`（结果机器单源）/ 其派生 `0_result_summaries.md` — **数据唯一源**，脚本拼装用 `val()` 取数。没有则先让用户生成或指给我（r-biostats 产出）。
...
- **数据有缺陷（缺失 / 需反推分母 / 口径不全 / 需近似）→ 先问能否补真实数据（年鉴 / 普查 / 标准人口库）、用户能提供什么；
  同时写进 BACKLOG。补全前论文按现有口径照常推进，补不全再问期望表述。NEVER 把缺陷自行写成"局限"或把清洗痕迹写进正文**（见全局 §3）。
...
| `references/chinese-thesis.md` | 写中文**学位论文**（硕/博）/部件 | 长文结构与 sections/ 分文件、各部件页数字数目标、前后置部件（封面/双声明/中英摘要/缩略语/综述/致谢/在读成果/附录）写法、长文分部件工作流、需人补加亮约定、与期刊论著差异表、学位论文自检 |
...
- 结果由 `r-biostats` 产出；统计图由 `publication-figures` 生成，流程/结构/机制、科学插图和图形摘要等非统计视觉由 `research-visuals` 按载体建立视觉简报、调用 imagegen，并仅在全部适用生成路径耗尽后最终回退 `svg-diagrams`，`xlsx` 出表；本技能只消费，不改分析。
...
## 九、改动后语言 / 逻辑一致性自检（强制要求 8 配套，每次生成或修改后必跑）
