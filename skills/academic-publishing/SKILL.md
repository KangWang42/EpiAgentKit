...
写作中发现但当轮补不了的缺口（某段需补一篇文献、缺某项数据无法下结论、某分析能强化论证但还没做、讨论里冒出的下一步设想），**立即追加一行到项目根 `BACKLOG.md` 待办表**（类别+内容+执行方+优先级+日期，见全局 CLAUDE.md §2）；占位符 `[待补充引用]` 只标"此处缺引用"，BACKLOG 才记"要去补什么、谁去补"——两者并用，不互相替代。
