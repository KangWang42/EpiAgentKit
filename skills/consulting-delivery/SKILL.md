...
description: |
  统计咨询交付包标准技能。把已完成的 R 或 Python 分析打包成客户可独立复现、可直接阅读、无模板化写作痕迹的专业交付物。
  触发场景：(1) 用户说"给客户做""咨询任务""交付给 XX""打包结果"；(2) 在 05_reports/ 下新建结果-M-D-主题 文件夹；(3) r-biostats 或项目专用 Python 分析完成后需要外发；(4) 用户请人做的分析要发回对方。
  上游依赖：biostat-principles（尤其原则 6 可复现）+ 已完成并验证的分析主流程 + academic-humanizer 学术审校。
---
...
| `r-biostats` 或项目 Python skill 完成分析 | 打包、改路径、写客户文档 | `academic-humanizer` 做事实锁、证据强度与学术语体审校 |
