...
def build_study_design():
    ...
    add_body(
        document,
        "主要分析人群包括满足纳入条件、具有基线暴露判定且至少有一次结局随访的研究对象；是否采用全分析集及其与完全病例分析的关系须在实施前冻结。首先按暴露组描述基线特征和样本流，连续变量根据分布报告均数与标准差或中位数与四分位数，分类变量报告人数与构成比。组间描述不以单因素检验结果决定协变量纳入；若报告传统的独立样本 t 检验，应同时报告效应量和区间估计。",
    )
    add_body(
        document,
        "主要模型拟采用线性回归估计 12 个月收缩压变化的调整后均值差，预设调整基线收缩压、年龄、性别、糖尿病、慢性肾病、药物类别、用药依从性和就诊频率。模型报告回归系数、95% 置信区间和必要的 P 值，检查残差分布、异方差和高影响观测。若结局分布或随访结构使线性模型不适用，任何替代模型均作为方案修订记录，不根据显著性选择。",
    )
...
def build_report():
    ...
    add_body(
        document,
        f"本次分析纳入 {n:,} 名固定模拟研究对象并观察到 {events:,} 个结局事件。调整年龄、性别、疾病分期和标准化生物标志物后，强化方案组相对于常规方案组的结局发生风险比为 HR {hr:.2f}（95% CI {low:.2f}～{high:.2f}，P < 0.001）。在该固定模拟数据和预设模型下，强化方案与较低的结局发生风险相关；该结果不构成真实临床证据，也不能被解释为已经证实的治疗因果效应。",
    )
...
def build_peer_review():
    ...
    add_numbered_item(
        document,
        "m2　P 值写法：",
        "统一 P < 0.001 等写法的间距和保留位数，并与正文、表格和图注保持一致。",
    )
