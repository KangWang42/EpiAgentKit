...
| results.yaml、DECISIONS、SESSION_LOG、conventions | `CLAUDE.md` §3；`biostat-principles/references/result-summary-schema.md` | 唯一来源指针常驻，schema 下沉且由 R/Python 共用 |
| BACKLOG 四列、状态与移出流程 | `project-hygiene.md` §6；`project-init` 模板 | 完整格式下沉，发现即记仍为硬红线 |
| error/warning/NaN 全量核验 | `CLAUDE.md` §4“执行与异常处理”；执行 skill 的 RUN/VERIFY | 保留为跨任务硬红线 |
...
| 多套优先级与记忆锚点 | 仅 `CLAUDE.md` §6 定义优先级；其他 skill 引用 | 消除相反排序，锚点由唯一来源指针覆盖 |
