...
release 1.1 只包含 `SKILLS_INCLUDED.txt` 列出的 18 个 skills、`CLAUDE.md`、使用说明和本说明。压缩包不包含用户配置、凭据、会话记录、缓存、研究数据、系统运行时或源仓库维护目录。
...
`research-visuals` 使用平台提供的 imagegen 能力；imagegen 服务、模型和系统 skill 不属于本包。`academic-ppt` 负责学术汇报内容与逐页结构，实际 PowerPoint 文件读写依赖平台或本机已有且使用者有权使用的文件处理能力。Word、PDF 与 Excel 亦遵循同一原则。本包不要求 LibreOffice，也不提供 R、Python、Node、Java、LibreOffice、TeX、Git、系统库或统计分析包。
