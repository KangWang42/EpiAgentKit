...
class WorkflowRoutingTests(unittest.TestCase):
    ...
    def test_skill_maintenance_contract_is_regression_safe(self) -> None:
        ...
        self.assertIn("对每项拟新增、保留或重写的要求做必要性判定", maintenance)
        self.assertIn("删除该要求是否会改变动作、决策、例外、停止条件或验收结果", maintenance)
        self.assertIn("若删除后行为不变，直接删除", maintenance)
        self.assertIn("不把已经自然成立的结果", maintenance)
    ...
    def test_publication_figures_separates_manuscript_figure_from_caption(self) -> None:
        ...
        self.assertNotIn("Times New Roman", skill)
