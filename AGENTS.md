...
On Windows, identify the active shell executable and version before relying on shell-specific behavior. Treat `powershell.exe` as Windows PowerShell 5.1 and use PowerShell 7 only when `pwsh` is actually available; do not install it or silently pass PowerShell 7-only scripts to 5.1. Set `[Console]::OutputEncoding = [System.Text.Encoding]::UTF8` and read text with `Get-Content -Encoding utf8`. Treat mojibake as a failed read. Use `-LiteralPath` only for cmdlets that support it, request terminating errors for critical cmdlets, and check native-command exit codes immediately. Put multiline logic, JSON, regular expressions, non-ASCII paths, or complex native arguments in a repository script under the appropriate workbench or persistent script directory. Avoid nested `powershell|pwsh -Command`, `Invoke-Expression`, string-built commands, and unverified `cmd.exe /c` or `Start-Process -ArgumentList` calls.

## Skill Maintenance

Every request to add, revise, repair, rename or remove a skill automatically uses `epiagentkit-maintenance` together with `skill-creator`; the user does not need to restate this requirement. Treat skill improvement as regression-safe optimization, not append-only documentation. Treat a reported word, file, or project failure as a representative case: define the shared cause and scan all affected workflows before editing, while preserving legitimate legal, technical, and domain-specific uses. Do not stop at a literal replacement or the named file. Before editing, identify the observed failure or new use case, behaviors that must remain unchanged, the smallest coherent change, and representative old and new evaluations. Classify existing material as keep, rewrite, merge, move, script, or delete before adding rules or files. Keep each concept in one source: global constraints in `CLAUDE.md`, core task selection and workflow in `SKILL.md`, conditional detail and examples in `references/`, and repeated deterministic operations in `scripts/`. Prefer the smaller change when two designs pass the same evaluations; remove stale, redundant, conflicting, speculative, or overly project-specific content. Do not trade away existing behavior under the label of simplification unless the user explicitly changes the governing rule.
...
Use Conventional Commits: `feat(scope): summary`, `fix(scope): summary`, `docs(scope): summary`, or `refactor(scope): summary`. Make each commit one coherent, reversible unit; use a specific scope and action-object summary, and add a body covering motivation, key behavior, validation, and compatibility for non-trivial changes. After a completed request passes validation, commit the coherent change automatically unless the user says not to commit, but only when Git is available and the current directory is a repository. New skill additions follow the pre-commit review gate in `epiagentkit-maintenance`; edits, repairs, renames and removals of existing skills use that gate only when the user explicitly requests review artifacts. After every successful commit in this repository, run `python scripts/epiagentkit.py sync --target all` followed by `python scripts/epiagentkit.py doctor --target all` so the local Claude Code and Codex installations match the committed source. If a post-commit check requires another commit, repeat synchronization and doctor after that commit. If Git is unavailable or the directory is not a repository, skip all Git operations; do not initialize a repository or install Git. Push only when the user explicitly requests it in the current turn; do not ask, remind, or batch-push on the user's behalf. Never force-push or rewrite remote history. Review the entire worktree before committing and do not sweep unexplained pre-existing changes into the commit. Pull requests must explain the problem, affected skills or hooks, verification commands, and compatibility effects. Link issues; include screenshots only for visual or document output.
